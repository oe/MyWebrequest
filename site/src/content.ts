...
export const homeCopy: Record<Locale, HomeCopy> = {
  en: {
    ...
    title: 'Redirect APIs. Block requests. Stay in control.',
    description:
      'Point an API at localhost, block unwanted requests, or change request headers for a specific site. Test URL patterns before enabling your rules. No account or cloud service.',
  ...
  'zh-CN': {
    ...
    title: '重定向 API，掌控请求。',
    description:
      '将 API 重定向到本地服务，拦截指定请求，或按网站修改请求头。先测试 URL 匹配，再启用规则，无需账号和云端服务。',
    quickStart: '快速上手',
  ...
  ko: {
    ...
    title: 'API를 로컬로 연결하고 요청을 제어하세요.',
    description:
      'API를 localhost로 연결하고, 원치 않는 요청을 차단하거나 특정 사이트의 요청 헤더를 바꾸세요. 규칙을 켜기 전에 URL 패턴을 테스트하세요. 계정이나 클라우드 서비스가 없습니다.',
    quickStart: '빠르게 시작하기',
  ...
  ja: {
    ...
    title: 'API をローカルへ。リクエストを自在に。',
    description:
      'API を localhost に向け、不要なリクエストをブロックし、サイトごとにリクエストヘッダーを変更。URL の一致を試してからルールを有効にできます。アカウントもクラウドも不要です。',
    quickStart: 'まず使ってみる',
  ...
  fr: {
    ...
    title: 'Redirigez vos API. Maîtrisez vos requêtes.',
    description:
      'Dirigez une API vers localhost, bloquez des requêtes ou modifiez les en-têtes pour un site précis. Testez les URL avant d’activer vos règles. Sans compte ni service cloud.',
  ...
  es: {
    ...
    title: 'Redirige API. Controla tus solicitudes.',
    description:
      'Dirige una API a localhost, bloquea solicitudes o cambia los encabezados de un sitio concreto. Prueba los patrones URL antes de activar las reglas. Sin cuenta ni servicio en la nube.',
