...
$(function($) {
  var TABNODATATR, addRule, deleteRules, dialogOKCB, getObjValues, getVcardString, hash, hideDialog, initRequestSection, isValueInObj, rules, showDialog, showTip;
  ...
  $('.tab-content').on('click', '.make-qrcode', function(e) {
    ...
    if ($this.is('textarea')) {
      ...
      if ($tab.length) {
        return $tab.text(str.length + '/300');
      }
    }
  });
  return $('.letter-cunt-wrapper').on('keyup', 'textarea', function(e) {
    return $(this).next().text(this.value.trim().length + '/300');
  });
});
