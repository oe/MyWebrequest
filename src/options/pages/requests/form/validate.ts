 ...
 * @param  {String}  url
 */
function checkChromeRule (url: string) {
 ...
 */
function checkParamFormat (url: string, useReg?: boolean) {
 ...
 */
function checkCustomMatchParams (url: string) {
 ...
 * @return {Boolean}     [description]
 */
function checkCustomMatchRule (url: string, useReg?: boolean) {
  url = url.trim()
 ...
 * @param  {string}  url
 */
function checkCustomMatchStrRule (url: string) {
 ...
 * @param  {String}  reg
 */
function checkCustomMatchRegRule (reg: string) {
  checkReg(reg)
 ...
 * @return {Boolean}
 */
function isRedirectHasNoParams (redirectUrl: string) {
  return !/\{[^}]+\}/.test(redirectUrl) && !/\*/.test(redirectUrl)
}

function checkCustomRedirectRule (redirectURL: string, matchURL: string, useReg?: boolean) {
 ...
 */
function isSubRule (rule: string, subRule: string) {
  let matches = utils.getURLParts(rule)!
  const prtl1 = matches[1]
  let url1 = matches[2] + matches[3]

  matches = utils.getURLParts(subRule)!
  const prtl2 = matches[1]
  const url2 = matches[2] + matches[3]
...
function isURLMatchPattern (url: string, pattern: string) {
  const regStr = pattern.replace(escapeRegExp, '\\$&').replace(/\*/g, '.*')
